...
async function runTaskInner(channelId, requestText) {
	...
	const promptText = requestText + (PREVIEW_ENABLED ? DESIGN_MOCKUP_INSTRUCTION : '');
	const firstPrompt = CLAUDE_SLASH_COMMAND ? `${CLAUDE_SLASH_COMMAND} ${promptText}` : promptText;
	const claudeArgs = isResume
		? ['-p', promptText, '--resume', sessionId, '--permission-mode', CLAUDE_PERMISSION_MODE]
		: ['-p', firstPrompt, '--session-id', sessionId, '--permission-mode', CLAUDE_PERMISSION_MODE];
	...
	if (changedFiles.length === 0) {
		...
		pendingTasks.set(channelId, { branch, sessionId, originalRequest, updatedAt: Date.now() });
		return {
			status: 'no-change',
	...
	if (PREVIEW_ENABLED && (!preview || !preview.pngPath) && touchesUi(changedFiles)) {
		try {
			await run(
				CLAUDE_BIN,
				[
